...
export class Humanoid {
    constructor(scene, x, y, z, model = 'Optimus') {
        ...
        this.parts = {}; // name → { mesh } — for component inspector clicks
        this.joints = {}; // kept for API compatibility
    ...
    init(x, z) {
        ...
        // ═══════════════════════════════════════════════
        // PHYSICS LAYER: Single capsule — fully locked rotation
        // ═══════════════════════════════════════════════
        const capHH  = 0.55 * s;   // half-height of capsule cylinder
        const capR   = 0.28 * s;   // radius
        // When resting on ground, capsule centre = capHH + capR above ground
        this.footOffset = (capHH + capR); // = 0.83*s — used in update() for visual sync
        const comY = this.footOffset + 0.5 * s; // spawn higher than rest so no initial ground clipping

        const bodyDesc = RAPIER.RigidBodyDesc.dynamic()
            .setTranslation(x, comY, z)
            .setLinearDamping(5.0)
            .setAngularDamping(999.9)
            .lockRotations();
        ...
        physics.world.createCollider(
            RAPIER.ColliderDesc.capsule(capHH, capR)
                .setMass(70 * spec.massMult)
                .setFriction(1.5)
                .setRestitution(0.0),
            this.physicsBody
        );
        ...
        this.group.position.set(t.x, t.y - this.footOffset, t.z);
    }
    ...
    update(delta, _gaitParams, pose, expertise = 1.0) {
        ...
        // (Removed previous misplaced recovery check)
            ...
            if (this.isJumping && vel.y < 0.1 && tp.y < this.footOffset + 0.2) {
                this.isJumping = false;
            }
        }
        ...
        if (speed > 0) {
            // Running gait
            this.hipLeftGroup  && (this.hipLeftGroup.rotation.x  = hipL);
            this.hipRightGroup && (this.hipRightGroup.rotation.x = hipR);
            this.kneeLeftGroup  && (this.kneeLeftGroup.rotation.x  = -kneeL);
            this.kneeRightGroup && (this.kneeRightGroup.rotation.x = -kneeR);
            this.ankleLeftGroup  && (this.ankleLeftGroup.rotation.x  = ankL);
            this.ankleRightGroup && (this.ankleRightGroup.rotation.x = ankR);
            this.shoulderLeftGroup  && (this.shoulderLeftGroup.rotation.x  = armL);
            this.shoulderRightGroup && (this.shoulderRightGroup.rotation.x = armR);
            this.elbowLeftGroup  && (this.elbowLeftGroup.rotation.x  = ebowL);
            this.elbowRightGroup && (this.elbowRightGroup.rotation.x = ebowR);
            if (this.headGroup) this.headGroup.rotation.x = 0;
        } else if (pose) {
            // Manual FK pose from sliders
            this.hipLeftGroup  && (this.hipLeftGroup.rotation.x  = pose.hipLeft   || 0);
            this.hipRightGroup && (this.hipRightGroup.rotation.x = pose.hipRight  || 0);
            this.kneeLeftGroup  && (this.kneeLeftGroup.rotation.x  = pose.kneeLeft  || 0);
            this.kneeRightGroup && (this.kneeRightGroup.rotation.x = pose.kneeRight || 0);
            this.ankleLeftGroup  && (this.ankleLeftGroup.rotation.x  = pose.ankleLeft  || 0);
            this.ankleRightGroup && (this.ankleRightGroup.rotation.x = pose.ankleRight || 0);
            this.shoulderLeftGroup  && (this.shoulderLeftGroup.rotation.x  = pose.shoulderLeft  || 0);
            this.shoulderRightGroup && (this.shoulderRightGroup.rotation.x = pose.shoulderRight || 0);
            this.elbowLeftGroup  && (this.elbowLeftGroup.rotation.x  = pose.elbowLeft  || 0);
            this.elbowRightGroup && (this.elbowRightGroup.rotation.x = pose.elbowRight || 0);
            if (this.headGroup) this.headGroup.rotation.x = pose.headPitch || 0;
        }
    }
}
